...
description: Periodic automated UX audit of SatuSatu's live site against the customer journey map criteria and trust signal checklist.
...
Runs a structured UX audit of SatuSatu's live site to catch regressions, broken flows, and missing trust signals. Compares the current live state against the expected state defined in the customer journey map (Page 01) and the dependency chains (Page 04). Designed to run weekly as a recurring check.

## Input

- **Target URL** — SatuSatu production URL (or staging URL for pre-release audit)
- **Audit scope** — Full journey (all 6 stages) or specific page type (homepage, search, listing, checkout)
...
3. **Check** — For each page, verify the following checklist:
...
**Input**: Full audit of satusatu.com (mobile viewport)
...
| Social Proof Counter | ❌ Fail | Not yet shipped (Initiative #10, P1) |
